...
class_weights_dict = {
    ...
    1: (total_samples / (2 * train_class_counts[1])) * 1.5 if train_class_counts[1] > 0 else 1.0
}

print("Calculated class weights (malignant boosted 1.5x):", class_weights_dict)
...
model_save_path = os.path.join(backend_dir, "breakhis_mobilenet_improved_model.keras")
...
threshold_path = os.path.join(backend_dir, "optimal_threshold.txt")
...
def plot_training_history(history, fine_tune_history):
    ...
    plt.tight_layout()
    save_path = os.path.join(backend_dir, 'training_history.png')
...
def plot_probability_distribution(y_true, y_pred_proba, threshold=0.5):
    ...
    plt.tight_layout()
    save_path = os.path.join(backend_dir, 'probability_distribution.png')
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
print(f"All outputs saved to: {backend_dir}")